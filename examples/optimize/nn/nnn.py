...
class RayTraceProblem(Problem):
    ...
    def _add_data_to_df(self, x: np.ndarray, results: np.ndarray):
        ...
        df = pd.DataFrame(np.column_stack((x, results, step)),
                          columns=self.indep_args_names + ["mean", "std", "cost", "step"])
        if self.df is None:
            self.df = df
        else:
...
def main():
    ...
    domain = [
        # continuous
        {"name": "num_rays", "type": "int", "value": 100_0},
        {"name": "number_lights", "type": "int", "min": 10, "max": 100},
        {"name": "mirrors", "type": "bool", "value": False},
        ...
        {'name': 'grid_type', 'type': 'discrete', 'items': 'spiral'},  # ['circle', 'ogrid', 'grid', 'spiral']
    ]

    problem = RayTraceProblem(domain, n_obj=3)
    algorithm = NSGA2(pop_size=27)
    ...
    df = pd.DataFrame(np.column_stack((res.X, res.F)), columns=problem.indep_args_names + ["mean", "std", "cost"])
