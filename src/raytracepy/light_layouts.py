...
class PointPattern(ABC):
    ...
    def add_to_plot(self, fig, **kwargs):
...
class GridPattern(PointPattern):
    ...
    def __init__(self,
        ...
        :param op_offset: will offset every other row
        ...
        self._xy_points: np.ndarray = np.empty((num_points, 2), dtype=GridPattern.number_type)
        self._get_points()
    ...
    def _set_counts(self, x_count, y_count, num_points):
        ...
        elif x_count is None and y_count is not None:
            self.x_count = 1
            self.y_count = y_count
            self.num_points = y_count
        else:
            raise AttributeError(f"Issue with 'x_count', 'y_count', or 'num_points'.")
